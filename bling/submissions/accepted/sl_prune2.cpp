...
ll solve4(int remdays, ll curbling, array<ll, 3> numNormal, array<ll, 3> numExotic) {
	...
	int normalLim = 4;
	if (numNormal[1] || numNormal[2]) {
		normalLim -= 1;
	}
	if (numNormal[1] && numNormal[2]) {
		normalLim -= 1;
	}
	if (numNormal[0] > normalLim) {
		add += gain * (numNormal[0] - normalLim) * 100;
		numNormal[0] = normalLim;
	}
...
ll solve2(int remdays, ll curbling, ll curnormal, ll curexotic, array<ll, 3> numNormal, array<ll, 3> numExotic) {
	...
	rep(sellnormal,0,12+1) rep(sellexotic,0,3+1) {
		...
		if (curbling + 100 * sellnormal + 500 * sellexotic > 400 * 3 + 500) {
			// Too much money, can just replant and wait 3 days instead.
			continue;
		}
