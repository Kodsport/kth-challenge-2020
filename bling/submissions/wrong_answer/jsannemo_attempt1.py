...
def sol(d, b, t, t0, t1, t2, e, e0, e1, e2):
    ...
    res = 0
    for esell in range(0, min(3, e+1)):
        for tsell in range(0, min(9, t+1)):
            if (esell or tsell) and D - d >= 10: continue
            tb = b + 500 * esell + 100 * tsell + t1 * 100 + e1 * 500 - 400
            tb2 = b + 500 * esell + 100 * tsell + (t1 + t2) * 100 + (e1 + e2) * 500 - 800
